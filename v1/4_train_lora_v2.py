...
def main():
    ...
    parser.add_argument("--epochs", type=int, default=5, help="训练轮数 (K=5建议 ≥5)")
    ...
    # 创建TrainingArguments的地方，添加 remove_unused_columns=False
    # training_args = TrainingArguments(
    #     output_dir="./output",
    #     per_device_train_batch_size=1,
    #     num_train_epochs=10,
    ...
    # 替换原 training_args = TrainingArguments(...)
    # training_args = TrainingArguments(
    #     per_device_train_batch_size=1,
    #     gradient_accumulation_steps=5,
    #     learning_rate=args.lr,
    ...
    #     save_strategy="epoch",
    #     fp16=False,
    #     # ✅ 新增三行（解决 AttributeError）
    #     ref_model_sync_steps=500,      # 关键：添加缺失属性
    #     max_length=512,
    ...
    #     # ==============================
    # )
    # 替换原 TrainingArguments
    training_args = TrainingArguments(
        per_device_train_batch_size=1,
        gradient_accumulation_steps=2, # 单卡最优值, 平衡批次与效率
        learning_rate=args.lr,
        num_train_epochs=args.epochs,
        max_grad_norm=0.5,
        logging_steps=1,
        output_dir=args.output,
        save_strategy="epoch",
        # ✅ 移除 ref_model_sync_steps（trl 0.7.11 不需要）
        report_to="none",
        bf16=True,
        gradient_checkpointing=True
    )
